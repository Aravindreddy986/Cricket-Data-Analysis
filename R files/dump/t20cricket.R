...
#######################################################
              #Loading a single file 211028# ####  few local declarations
  ...
  
{
  ...
  cricketcsvattestt4 <- subset(cricketcsvattestt3, cricketcsvattestt3$meta == innings1stor2nd[1,2])

  ### fetching the only balls/overs played by all players
  oversact <-   sqldf(" select * from cricketcsvattestt4 c  where c.type4  = 'batsman' ")
  ...
  for (i in 1:dim(playersactwhobatted)[1])
  {
    ...
    runscored <- sqldf(" select  case when sum(c.`1stinningsinfo`) is Null then 0 else sum(c.`1stinningsinfo`) end from cricketcsvattestt4 c
    ...
    alldetails <- sqldf(" select * from cricketcsvattestt4 c where  c.overs in ( select oversplayed  from oversactt2) ")
